...
function Fruit(image, x, y, width) {
    ...
    this.x = x;
    this.y = y;
    this.speed = generateRandomNumber(5, 10);
    this.width = width;
    this.clicked = false;
    this.complete = false;
...
function createRandomFruit() {
    // set random time for the next fruit 
    randomCreationTime = generateRandomNumber(1, 3);
    if (fruits.length < FRUIT_COUNT) {
        let randomFruit = fruitsList[generateRandomNumber(0, fruitsList.length - 1)];
...
canvas.addEventListener(events[deviceType].down,
    function (e) {
        let clickX = (isTouchDevice() ? e.touches[0].pageX : e.pageX) - canvas.offsetLeft;
        let clickY = (isTouchDevice() ? e.touches[0].pageY : e.pageY) - canvas.offsetTop;
